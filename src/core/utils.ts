...
export const generateEMWProductUrl = (id: string, name: string, seo: string) => {
  const temp = seo === "" || seo === undefined || seo === null ? name : seo
  return `/product/${slugify(temp)}/${id}/`;
}
export const generateEMWProductSeoUrl = (id: string, name: string, seo: string) => {
  const temp = seo === "" || seo === undefined || seo === null ? name : seo;
  const url= (temp=="/") ? '/' : `/${slugify(temp)}`;
  return url;
}
...
export const generateEMWCategoryUrl = (id: string, name: string) =>
  `/category/${slugify(name)}/${getDBIdFromGraphqlId(id, "EMWCategory")}/`;

export const generateEMWCategorySeoUrl = (name:string, seo: string) => {
  const temp = seo === "" || seo === undefined || seo === null ? name : seo
  return `/${slugify(temp)}`;
}
